...
public class NetUtils {
    ...
    public  void getDataAsynFromNet(String url, String cookie ,final MyNetCall myNetCall) {
        //1 构造Request
        Request.Builder builder = new Request.Builder();
        Request request=builder.get().url(url).addHeader("Cookie", cookie).build();
        //2 将Request封装为Call
        Call call = mOkHttpClient.newCall(request);
        //3 执行Call
        call.enqueue(new Callback() {
     ...
     * @param myNetCall
     */
    public  void postJsonDataAsynToNet(String url, String json, final MyNetCall myNetCall) {
        //1构造RequestBody
        MediaType JSON = MediaType.parse("application/json; charset=utf-8");
        RequestBody body=RequestBody.create(JSON,json);
        //2 构造Request
        Request.Builder requestBuilder = new Request.Builder();
        Request request = requestBuilder.post(body).addHeader("Authorization","Bearer 2f1c86ac4969c59b336922422c8ecd25").url(url).build();
        //3 将Request封装为Call
        Call call = mOkHttpClient.newCall(request);
        //4 执行Call
        call.enqueue(new Callback() {
